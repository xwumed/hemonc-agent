...
def build_doctor_agents():
    ...
    internal_agent = Agent[MedicalContext](
        ...
        model_settings=build_model_settings(tool_choice="required", max_completion_tokens=1536),  # 增加token限制
...
async def generate_pubmed_queries(context_text: str) -> List[str]:
    ...
    resp = await external_client.chat.completions.parse(
        ...
        max_completion_tokens=2048,  # 增加token限制以确保完整响应
...
chairman = Agent[MedicalContext](
    ...
    model=OpenAIChatCompletionsModel(model=MODEL_NAME, openai_client=external_client),
    model_settings=build_model_settings(max_completion_tokens=2048),  # 增加token限制以支持完整汇总
...
"""
    ...
    resp = await external_client.chat.completions.create(
        ...
        max_completion_tokens=500,  # 增加token限制
    )
    
    # 添加空值检查
    content = resp.choices[0].message.content
    if content is None:
        logger.warning(f"LLM returned None for {heading} summary, using original text")
        # 如果模型返回None，返回原文的前200个单词
